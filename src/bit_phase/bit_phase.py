...
def bytes_to_bits(data: bytes) -> bit_array:
...
def bits_to_bytes(bits: bit_array) -> bytes:
    if bits.size == 0:
        return b""
...
def int_to_bit_list(x: int, bits: int = 16) -> bit_array:
    shifts = np.arange(bits - 1, -1, -1, dtype=np.int64)
    return ((x >> shifts) & 1).astype(np.int8)


def bits_to_int(bits: bit_array) -> int:
    bits = np.asarray(bits, dtype=np.int8).reshape(-1)
    if bits.size == 0:
        return 0
